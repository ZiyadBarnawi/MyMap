...
const goToWorkout = function (entry) {
  map.setView([entry.dataset.lat, entry.dataset.lng], mapZoomLvl, {
    animate: true,
    pan: { duration: 1 },
  });
};
  ...
  function (position) {
    ...
    for (let entry of entriesStrings) {
      ...
      if (restoredElement.classList.contains("running-entry")) {
        console.log(
          restoredElement.dataset.date,
          restoredElement.dataset.lat,
          restoredElement.dataset.lng,
          restoredElement.dataset.id,
...
form.addEventListener("submit", function (e) {
  ...
  if (typeInput.value === "running") {
    ...
    L.marker([lat, lng], { riseOnHover: true })
      ...
      .bindPopup(
        L.popup([lat, lng], {
          content: `<p  style="color:black; font-size: 18px;"> running in ${date.getDate()} of ${date.toLocaleString(
            "default",
            { month: "long" }
          )}<p/>`,
          maxWidth: 250,
    ...
    verticalContainer.insertAdjacentHTML(
      ...
      `<div data-date="${date.getDate()} of ${date.toLocaleString(
        ...
        durationInput.value
      }  minutes 🏃‍➡️ <svg class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" fill="none"/><polyline points="208 96 128 176 48 96" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16"/></svg> <span class="hidden-span"> Candece: ${
        candeceInput.value
      } </span> </p>
            </div>`
    );
    ...
    L.marker([lat, lng], { riseOnHover: true })
      ...
      .bindPopup(
        L.popup([lat, lng], {
          content: `<p  style="color:black; font-size: 18px;"> cycling in ${date.getDate()} of ${date.toLocaleString(
            "default",
            { month: "long" }
          )}<p/>`,
          maxWidth: 250,
    ...
    verticalContainer.insertAdjacentHTML(
      ...
      `<div data-date="${date.getDate()} of ${date.toLocaleString(
        ...
        durationInput.value
      } minutes 🚴 <svg class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" fill="none"/><polyline points="208 96 128 176 48 96" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16"/></svg> <span class="hidden-span"> Elevation gain: ${
        elevGainInput.value
      } </span> </p>
            </div>`
    );
